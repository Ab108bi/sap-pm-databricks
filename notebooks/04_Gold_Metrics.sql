...
-- MAGIC
-- MAGIC **Note:** The `gold_orders_fact` table performs all type casting
-- MAGIC (string → double for numerics, string → date/timestamp for dates).
-- MAGIC Downstream Gold tables inherit the properly-typed columns.
-- MAGIC
-- MAGIC **Run order:** Cell 1 (gold_orders_fact) must run FIRST, then
-- MAGIC Cells 2-5 can run in any order.

-- COMMAND ----------

-- DBTITLE 1,Cell 1 — Dashboard-ready order-level fact table (MUST RUN FIRST)
...
SELECT
  ...
  -- Date dimensions (string → date)
  TO_DATE(Created_Date, 'yyyy-MM-dd') AS created_date,
  DATE_FORMAT(TO_DATE(Created_Date, 'yyyy-MM-dd'), 'yyyy-MM') AS year_month,
  DATE_FORMAT(TO_DATE(Created_Date, 'yyyy-MM-dd'), 'yyyy') AS year,
  DATE_FORMAT(TO_DATE(Created_Date, 'yyyy-MM-dd'), 'MMMM') AS month_name,
  WEEKOFYEAR(TO_DATE(Created_Date, 'yyyy-MM-dd')) AS week_of_year,
  _file_plant, _file_month, _file_year, _file_week,

  Planned_Start_Date, Planned_End_Date,
  Actual_Start_Date, Actual_End_Date,

  -- Cost metrics (string → double)
  CAST(Planned_Cost_USD AS DOUBLE) AS Planned_Cost_USD,
  CAST(Actual_Cost_USD AS DOUBLE) AS Actual_Cost_USD,
  ROUND(CAST(Actual_Cost_USD AS DOUBLE) - CAST(Planned_Cost_USD AS DOUBLE), 2) AS cost_variance_usd,
  ROUND(
    CASE WHEN CAST(Planned_Cost_USD AS DOUBLE) > 0
    THEN (CAST(Actual_Cost_USD AS DOUBLE) - CAST(Planned_Cost_USD AS DOUBLE)) / CAST(Planned_Cost_USD AS DOUBLE) * 100
    END, 1
  ) AS cost_variance_pct,

  -- Labor metrics (string → double)
  CAST(Planned_Labor_Hours AS DOUBLE) AS Planned_Labor_Hours,
  CAST(Actual_Labor_Hours AS DOUBLE) AS Actual_Labor_Hours,
  ROUND(CAST(Actual_Labor_Hours AS DOUBLE) - CAST(Planned_Labor_Hours AS DOUBLE), 1) AS labor_variance_hours,
  ROUND(
    CASE WHEN CAST(Planned_Labor_Hours AS DOUBLE) > 0
    THEN CAST(Planned_Labor_Hours AS DOUBLE) / NULLIF(CAST(Actual_Labor_Hours AS DOUBLE), 0) * 100
    END, 1
  ) AS labor_efficiency_pct,

  -- Duration metrics in hours (string → timestamp → difference)
  ROUND(
    CASE WHEN Actual_Start_Date IS NOT NULL AND Actual_Start_Date != ''
      AND Actual_End_Date IS NOT NULL AND Actual_End_Date != ''
    THEN (UNIX_TIMESTAMP(Actual_End_Date, 'yyyy-MM-dd HH:mm') - UNIX_TIMESTAMP(Actual_Start_Date, 'yyyy-MM-dd HH:mm')) / 3600
    END, 1
  ) AS actual_duration_hours,
  ROUND(
    (UNIX_TIMESTAMP(Planned_End_Date, 'yyyy-MM-dd HH:mm') - UNIX_TIMESTAMP(Planned_Start_Date, 'yyyy-MM-dd HH:mm')) / 3600
  , 1) AS planned_duration_hours,
  ...
  CASE
    ...
    WHEN CAST(Actual_Cost_USD AS DOUBLE) <= CAST(Planned_Cost_USD AS DOUBLE) THEN 'Under Budget'
    WHEN CAST(Actual_Cost_USD AS DOUBLE) <= CAST(Planned_Cost_USD AS DOUBLE) * 1.1 THEN 'Within 10%'
    ELSE 'Over Budget'
...
-- DBTITLE 1,Cell 2 — Plant KPI scorecard
...
SELECT
  ...
  -- Key ratios
  ROUND(
    SUM(CASE WHEN maintenance_category = 'Preventive' THEN 1.0 ELSE 0 END)
    / NULLIF(SUM(CASE WHEN maintenance_category = 'Corrective' THEN 1.0 ELSE 0 END), 0)
  , 2) AS pm_cm_ratio,
  ROUND(SUM(is_breakdown) * 100.0 / COUNT(*), 1) AS breakdown_rate_pct,

  -- Schedule compliance
  ROUND(
    SUM(CASE WHEN schedule_status = 'On Time' THEN 1 ELSE 0 END) * 100.0
    / NULLIF(SUM(CASE WHEN schedule_status IN ('On Time','Late') THEN 1 ELSE 0 END), 0)
  , 1) AS schedule_compliance_pct,
  ...
  ROUND(SUM(CASE WHEN budget_status = 'Over Budget' THEN 1 ELSE 0 END) * 100.0
    / NULLIF(SUM(CASE WHEN Actual_Cost_USD IS NOT NULL THEN 1 ELSE 0 END), 0)
  , 1) AS pct_orders_over_budget,
...
-- DBTITLE 1,Cell 3 — Monthly trends
CREATE OR REPLACE TABLE workspace.sap_pm.gold_monthly_trends AS
SELECT
  Plant, Plant_Name, year_month,
  maintenance_category, Order_Type_Desc,
...
-- DBTITLE 1,Cell 4 — Failure & reliability analysis
...
-- DBTITLE 1,Cell 5 — Equipment hot spots
CREATE OR REPLACE TABLE workspace.sap_pm.gold_equipment_summary AS
SELECT
  Plant, Plant_Name,
  Equipment_ID, Functional_Location,
  COUNT(*) AS total_orders,
  SUM(is_breakdown) AS breakdowns,
  SUM(CASE WHEN maintenance_category = 'Corrective' THEN 1 ELSE 0 END) AS corrective_orders,
  ROUND(SUM(Actual_Cost_USD), 0) AS total_cost,
